---
title: 향상된 Adobe Journey Optimizer의 보고 기능
description: Adobe Journey Optimizer에서 새로운 보고 및 오케스트레이션 측정 기능을 사용할 수 있습니다. 또한 Journey Optimizer의 향상된 기능에는 추가 데이터 소스, 고급 사용자 지정, 데이터 조작, 분석 등을 통해 Customer Journey Analytics와의 상호 운용성 향상 및 가치 상승이 포함됩니다.
feature: Experience Platform Integration
role: User
level: Intermediate
doc-type: Feature Video
duration: 22500
last-substantial-update: 2024-06-24T00:00:00.000Z
jira: KT-15767
exl-id: 9a3e1d44-e61f-416e-a4a2-c46bce7445c3
TQID: https://experienceleague.adobe.com/uOUi2EW5FQqCx5rNErENkI4Sq1JMUhV6mgDkIwcZPyo
product_v2: id: e98b7246-966c-4318-9e95-cad2f7a17dc7
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
topic_v2: id: aa2f3246-cb95-4b30-8899-fdf7d73550ccid: c2be0313-b3ae-45e0-b454-d20bf54b23f2
source-git-commit: fb8bcbdd846b74e46321c69b4ccee3752cbea5d4
workflow-type: tm+mt
source-wordcount: 112
ht-degree: 78%

---

# 향상된 Adobe Journey Optimizer의 보고 기능

Adobe Journey Optimizer에서 새로운 보고 및 오케스트레이션 측정 기능을 사용할 수 있습니다. 또한 Journey Optimizer의 향상된 기능에는 추가 데이터 소스, 고급 사용자 지정, 데이터 조작, 분석 등을 통해 Customer Journey Analytics와의 상호 운용성 향상 및 가치 상승이 포함됩니다. 자세한 설명서는 [새 보고 인터페이스 시작](https://experienceleague.adobe.com/ko/docs/journey-optimizer/using/channel-report/report-gs-cja){target="_blank"}을 참조하십시오.

>[!VIDEO](https://video.tv.adobe.com/v/3430413/?learn=on)
