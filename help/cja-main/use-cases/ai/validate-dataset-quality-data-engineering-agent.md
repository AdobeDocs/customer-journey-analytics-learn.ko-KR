---
title: Data Engineering Agent을 사용하여 데이터 세트 품질 유효성 검사
description: Data Engineering Agent의 데이터 유효성 검사 스킬을 사용하여 Adobe Customer Journey Analytics 데이터 세트를 빠르게 프로필링하고 유효성을 검사하는 방법을 알아봅니다.
feature: AI Tools
topic: Artificial Intelligence
role: Developer, User
level: Intermediate
doc-type: Feature Video
duration: 280
last-substantial-update: 2026-05-04T00:00:00Z
jira: KT-20744
source-git-commit: 3324740cfa93ad26145ee16052e5bf3694720678
workflow-type: tm+mt
source-wordcount: '93'
ht-degree: 0%

---

# Data Engineering Agent을 사용하여 데이터 세트 품질 유효성 검사

Data Engineering Agent의 데이터 유효성 검사 스킬을 사용하여 Adobe Customer Journey Analytics 데이터 세트를 빠르게 프로필링하고 유효성을 검사하는 방법을 알아봅니다. Customer Journey Analytics에서 보고서 및 여정을 작성하기 전에 잘못된 값을 감지하고 필드 수준 패턴을 이해하고 AI 기반 인사이트를 활용하여 데이터 품질을 개선하는 방법을 살펴볼 수 있습니다.

>[!VIDEO](https://video.tv.adobe.com/v/3486277/?learn=on&enablevpops)
