---
title: 미리보기를 사용하여 데이터 및 ID 결합 품질 확인
description: 연결을 구성할 때 직접 개인 ID 범위 및 영구 ID 범위를 포함하여 즉각적인 ID 결합 지표를 제공하는 Customer Journey Analytics의 데이터 세트 미리보기 기능에 대해 알아봅니다.
feature: Identity
role: Admin
level: Intermediate
doc-type: Feature Video
duration: 349
last-substantial-update: 2026-03-24T00:00:00Z
jira: KT-20509
source-git-commit: c4adc96faebf26fd3e0e58fe38b2a0a7f3c2d5c4
workflow-type: tm+mt
source-wordcount: '115'
ht-degree: 5%

---

# 미리보기를 사용하여 데이터 및 ID 결합 품질 확인

연결을 구성할 때 직접 개인 ID 범위 및 영구 ID 범위를 포함하여 즉각적인 ID 결합 지표를 제공하는 Customer Journey Analytics의 데이터 세트 미리보기 기능에 대해 알아봅니다.

수집 전에 신뢰할 수 있는 크로스 디바이스 및 크로스 채널 보고를 위해 데이터 세트 적합성을 평가하여 가치 실현 시간을 획기적으로 단축하고 실시간 통찰력을 통해 ID 전략 의사 결정의 위험을 제거합니다.

자세한 내용은 [설명서](https://experienceleague.adobe.com/en/docs/analytics-platform/using/stitching/validate){target="_blank"}를 검토하십시오.

>[!VIDEO](https://video.tv.adobe.com/v/3481892/?learn=on&enablevpops)
